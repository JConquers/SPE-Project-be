pipeline {
  ...
  stages {
    ...
    stage('Prepare Metadata') {
      steps {
        script {
          ...
          // Fetch useful git metadata for email
    ...
    //   steps {
    //     sh """
    //       kubectl -n dev set image deployment/backend backend=${IMAGE}
    //     """
    //   }
    // }
  }
  ...
  post {
    ...
    always {
      echo "Pipeline finished. See console output for details."
    }

    success {
      emailext(
        subject: "SUCCESS: Backend Deployment #${env.BUILD_NUMBER}",
        body: """
    ...
    failure {
      emailext(
        subject: "FAILURE: Backend Deployment #${env.BUILD_NUMBER}",
        body: """
